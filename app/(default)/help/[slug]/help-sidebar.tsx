        ...
        <ul>
          {links.map((link, linkIndex) => (
            <li key={linkIndex} className="py-2 border-b border-gray-800">
              <Link className={`flex items-center px-3 group transition duration-150 ease-in-out ${pathname === link.href ? 'text-gray-600' : 'text-gray-400 hover:text-gray-600'}`} href={link.href}>
                <span>{link.name}</span>
                <svg className="w-3 h-3 fill-current shrink-0 ml-2 opacity-0 group-hover:opacity-100 group-hover:text-gray-600 group-hover:translate-x-1 transition duration-150 ease-in-out transform" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg">
